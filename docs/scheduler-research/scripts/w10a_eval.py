"""Week 10A evaluation — W9 + PBRS, filtered test split."""
...
from w9_train import (
    ...
    D_CAND, D_ATTN, D_V, N_QT, N_ACTIONS,
    _LOG_DENOM,
)
...
if __name__ == "__main__":
    ...
    _print_stats("a) W10A AttentionDQN+PBRS (5-feature) — trace test", _stats(agent_mcts))
    ...
    print(f"{'W10A AttentionDQN+PBRS':<30} | {as_['mean']:>9.4f}s | {as_['std']:>8.4f}s | {as_['mean']-rs['mean']:>+8.4f}s | {as_['mean']-ss['mean']:>+9.4f}s")
    print(f"{'W9 AttentionDQN (5-ft)':<30} | {'19.2234s':>9} | {'11.5843s':>8} | {'-2.0870s':>8} | {'+4.0254s':>9}")
