"""Week 10B evaluation — learnable beta, filtered test split."""
...
from w10b_train import AttentionDQN10B
...
def _get_attn(agent, sv, action):
    ...
    _, cache = agent._attention_forward(ce, comp_encs, comp_valid)
...
if __name__ == "__main__":
    ...
    print("\nLoading W10B agent ...")
    agent = AttentionDQN10B(lr=0.001, gamma=1.0, grad_clip=1.0)
    agent.load(WEIGHTS_PATH)
    agent.epsilon = 0.0
    print(f"  Loaded. Final beta = {agent.beta():.6f}  (log_beta = {agent.log_beta[0]:.6f})")
    ...
    _print_stats("a) W10B AttentionDQN learnable beta — trace test", _stats(agent_mcts))
    ...
    print(f"{'W10B learnable beta':<30} | {as_['mean']:>9.4f}s | {as_['std']:>8.4f}s | {as_['mean']-rs['mean']:>+8.4f}s | {as_['mean']-ss['mean']:>+9.4f}s")
    print(f"{'W9 fixed beta=0.25':<30} | {'19.2234s':>9} | {'11.5843s':>8} | {'-2.0870s':>8} | {'+4.0254s':>9}")
    ...
    while len(arec) < 1000:
        ...
        while not done:
            ...
            action = agent.select_action(sv, epsilon=0.0, valid_actions=valid)
            if n_active >= 3:
                weights, cb, cv = _get_attn(agent, sv, action)
                if cv.sum() >= 2:
                    vw = weights.copy(); vw[~cv] = -1.0
                    top = int(np.argmax(vw))
                    vb  = [(cb[j], j) for j in range(4) if cv[j]]
                    si  = min(vb, key=lambda x: x[0])[1]
                    li  = max(vb, key=lambda x: x[0])[1]
                    arec.append({"top_s": top==si, "top_l": top==li,
                                 "ws": float(weights[si]), "wl": float(weights[li])})
            _, _, done, _ = env.step(action)
            sv = _encode_state(env, tasks)

    n_a = len(arec); ts = sum(r["top_s"] for r in arec); tl = sum(r["top_l"] for r in arec)
    mws = np.mean([r["ws"] for r in arec]); mwl = np.mean([r["wl"] for r in arec])
    print(f"\n{'='*60}")
    ...
    cb = float(np.expm1(0.20 * _LOG_DENOM)); cp = float(np.expm1(0.60 * _LOG_DENOM))
