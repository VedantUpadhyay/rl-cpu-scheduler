"""Week 9 evaluation — 5-dim AttentionDQN9, filtered trace test split."""
...
def _get_attention_weights(agent, sv, action, tasks, env):
    states   = sv[None].astype(np.float64)          # (1, 25)
    ...
    comp_bursts = comp_encs[0, :, 0]                # (4,) log-norm remaining
...
if __name__ == "__main__":
    ...
    print(f"\nLoading W9 agent from {WEIGHTS_PATH} ...")
    ...
    _print_stats("a) W9 AttentionDQN (5-feature, filtered) — trace test", _stats(agent_mcts))
    ...
    # ---- c) Round Robin ----
    print(f"\nRunning Round Robin (n={N_EVAL}, quantum={QT_W9[1]}s) ...")
    rr_mcts = np.array([_run_policy(rr, procs) for _, procs in all_eps])
    _print_stats(f"c) Round Robin (tier1={QT_W9[1]}s) — filtered trace test", _stats(rr_mcts))
    ...
    print(f"{'W9 AttentionDQN (5-ft)':<30} | {as_['mean']:>9.4f}s | {as_['std']:>8.4f}s | {as_['mean']-rs['mean']:>+8.4f}s | {as_['mean']-ss['mean']:>+9.4f}s")
    print(f"{'W8c AttentionDQN (3-ft)':<30} | {'23.3000s':>9} | {'10.8500s':>8} | {'+1.5300s':>8} | {'+7.5800s':>9}")
    ...
    # ---- e) Attention diagnostic (n_active >= 3) ----
    ...
    while len(attn_records) < 1000:
        ...
        while not done:
            ...
            action = agent.select_action(sv, epsilon=0.0, valid_actions=valid)
            if n_active >= 3:
                weights, comp_bursts, comp_vm = _get_attention_weights(agent, sv, action, tasks, env)
                if comp_vm.sum() >= 2:
    ...
    # Candidate: burst_norm=0.20, cpu_norm=0.125, mem_norm=0.339
    # Competitor: burst_norm=0.60, cpu_norm=0.125, mem_norm=0.339
    cand_actual_burst = float(np.expm1(0.20 * _LOG_DENOM))
    comp_actual_burst = float(np.expm1(0.60 * _LOG_DENOM))
    probe = np.zeros(N_PROCESSES * D_CAND, dtype=np.float64)
    probe[0*D_CAND:0*D_CAND+D_CAND] = [0.20, 1.0, 0.0, 0.125, 0.339]  # P0 candidate
    probe[1*D_CAND:1*D_CAND+D_CAND] = [0.60, 1.0, 0.0, 0.125, 0.339]  # P1 competitor
    ...
    print(f"   Candidate P0: burst_norm=0.20 → {cand_actual_burst:.2f}s, cpu_norm=0.125, mem_norm=0.339")
    print(f"   Competitor P1: burst_norm=0.60 → {comp_actual_burst:.2f}s, cpu_norm=0.125, mem_norm=0.339")
    print(f"{'='*60}")
