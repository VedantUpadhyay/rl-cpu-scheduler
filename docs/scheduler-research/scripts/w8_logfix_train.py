"""W8b training: AttentionDQN with log-normalization, recalibrated REWARD_SCALE, lambda=0.30."""
...
# ---------------------------------------------------------------------------
# FIX 1 — Log-normalization constant (replaces burst / BURST_P95)
# ---------------------------------------------------------------------------
_LOG_BURST_DENOM: float = float(np.log1p(BURST_P95))   # log(1 + 397) ≈ 5.988

# FIX 2 — Recalibrated REWARD_SCALE
REWARD_SCALE: float = float(N_PROCESSES * QUANTUM_TIERS[2])  # 5 × 8.0 = 40.0

# FIX 3 — Entropy annealing: 0.30 → 0.005 (was 0.10 → 0.005)
LAMBDA_START = 0.30
LAMBDA_END   = 0.005

# ---------------------------------------------------------------------------
# Other constants (unchanged from W7/W8)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# FIX 1 applied: log-normalized state encoding
# ---------------------------------------------------------------------------

def _log_norm_burst(burst: float) -> float:
    """FIX 1: log(1+burst) / log(1+BURST_P95)."""
    return float(np.log1p(burst) / _LOG_BURST_DENOM)


def _encode_state(env: SchedEnv) -> np.ndarray:
    """15-dim state vector with log-normalized burst durations.

    Per process (3 features × 5):
        log(1+remaining) / log(1+BURST_P95)   (FIX 1)
        arrived_flag
        wait_time / WAIT_NORM
    """
    vec: list[float] = []
...
def train() -> AttentionDQN:
    ...
    for b in [3, 10, 47, 397]:
        print(f"  burst={b:>4}s -> log-norm={_log_norm_burst(b):.4f}")
    print(f"  REWARD_SCALE = {REWARD_SCALE}  (N={N_PROCESSES} × tier2={QUANTUM_TIERS[2]})")
    ...
    with open(LOG_PATH, "w", newline="") as log_f:
        ...
        for ep in range(1, N_EPISODES + 1):
            # FIX 3: annealing starts at 0.30 (was 0.10)
            ...
            while not done:
                ...
                # FIX 2: use recalibrated REWARD_SCALE (40.0 not 100.0)
            ...
            if ep % PRINT_EVERY == 0:
                ...
                if ep == 5000 and ah > ENTROPY_GATE:
                    print(f"\nSTOP GATE: avg_H={ah:.4f} > {ENTROPY_GATE} at ep 5000 — not sharpening.")
                    return agent
...
if __name__ == "__main__":
    ...
    print("=" * 64)
    print("Week 8b — AttentionDQN, log-norm + REWARD_SCALE=40 + lambda=0.30")
    print("=" * 64)
    t0    = time.time()
    agent = train()
